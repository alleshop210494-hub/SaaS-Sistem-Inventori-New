...
export function InventoryDashboard() {
  ...
  const exportDashboardToExcel = () => {
    ...
    // Menggunakan format Tab-Separated (.xls) agar otomatis terbagi ke dalam kolom dengan rapi di Excel
    let excelRows = [];

    // Judul Laporan
    excelRows.push("LAPORAN RINGKASAN EKSEKUTIF INVENTORI\t\t\t\t\t\t");
    excelRows.push(`Tanggal Cetak: ${new Date().toLocaleString('id-ID')}\tTotal Jenis Barang: ${items.length} Item\t\t\t\t\t`);
    excelRows.push(""); // Baris kosong pemisah

    // Header Tabel
    excelRows.push(["No", "Nama Barang", "Kategori", "SKU", "Stok", "Harga Satuan (Rp)", "Total Nilai (Rp)"].join("\t"));
    ...
    excelRows.push("");
    excelRows.push(`\t\t\t\tTotal Keseluruhan Nilai Inventori:\t\t${totalValue}`);
